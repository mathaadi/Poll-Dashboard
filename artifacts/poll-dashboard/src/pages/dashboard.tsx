...
import { Users, Presentation, Star, Award, ChevronRight, MessageSquare, GraduationCap, BookOpen, LayoutGrid, BarChart2 as BarChart2Icon } from "lucide-react";
import { 
...
type DashboardTab = "overview" | "instructors" | "topics";

function RatingBadge({ value }: { value: number }) {
  const color =
    value >= 4.2 ? "bg-green-500/20 text-green-400" :
    value >= 3.5 ? "bg-amber-500/20 text-amber-400" :
...
export default function Dashboard() {
  ...
  const { data: instructors, isLoading: instrLoading } = useGetInstructors();
  ...
  const kpis = [
    ...
    { label: "Instructors", value: summary.total_instructors, icon: GraduationCap, color: "text-cyan-400" },
    { label: "Topics", value: summary.total_topics, icon: BookOpen, color: "text-violet-400" },
  ];
  ...
  const instructorChartData = (instructors || [])
    .slice(0, 10)
    .map((inst) => ({
      name: inst.instructor.length > 16 ? inst.instructor.slice(0, 16) + "…" : inst.instructor,
      fullName: inst.instructor,
      Delivery: inst.avg_delivery,
      Content: inst.avg_content,
    }));
  ...
  return (
    <div className="space-y-8 pb-20">
        ...
        {kpis.map((kpi, i) => {
          ...
          return (
            <motion.div 
              ...
              className="bg-card border border-border/50 rounded-2xl p-4 relative overflow-hidden group hover:border-primary/50 transition-colors"
            >
              <div className="absolute top-0 right-0 -mt-4 -mr-4 w-20 h-20 bg-gradient-to-br from-white/5 to-transparent rounded-full blur-2xl group-hover:bg-primary/10 transition-colors" />
      ...
      {/* Tab Navigation */}
      <div className="flex bg-card border border-border/50 rounded-xl p-1 self-start w-fit">
        {([
          ...
          <button
            key={id}
            onClick={() => { setActiveTab(id); setSelectedSubject(null); }}
            className={cn(
              "px-5 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all",
              activeTab === id ? "bg-white/10 text-white shadow" : "text-muted-foreground hover:text-white"
            )}
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {subjects?.map((sub, i) => (
                  ...
                  >
                    ...
                    {(sub as any).instructors?.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-border/30">
                        <p className="text-xs text-muted-foreground">
                          {(sub as any).instructors.join(", ")}
                        </p>
                      </div>
                    )}
        ...
        {activeTab === "instructors" && (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-muted-foreground">{(instructors || []).length} instructors</span>
            </div>
            ...
            {instrLoading ? (
              ...
              <div className="bg-card border border-border/50 rounded-2xl p-12 text-center">
                ...
                <h3 className="text-lg font-semibold text-white mb-2">No Instructor Data</h3>
                <p className="text-muted-foreground text-sm max-w-sm mx-auto">
                  Upload sessions with Instructor Name filled in to see per-instructor analytics. You can also edit existing uploads to add instructor info.
                </p>
              </div>
            ) : (
              <>
                {/* Instructor comparison chart */}
                {instructorChartData.length > 1 && (
                  <div className="bg-card border border-border/50 rounded-2xl p-6">
                    <p className="text-sm text-muted-foreground mb-4 font-medium">Delivery vs Content — all instructors</p>
                    <div className="h-[220px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={instructorChartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" />
                          <XAxis dataKey="name" stroke="hsl(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} />
                          <YAxis domain={[0, 5]} stroke="hsl(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} dx={-8} />
                          <RechartsTooltip
                            cursor={{ fill: 'rgba(255,255,255,0.05)' }}
                            contentStyle={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))', borderRadius: '12px', color: '#fff' }}
                            formatter={(val, name, props) => [val, props.payload.fullName ? `${name} — ${props.payload.fullName}` : name]}
                          />
                          <Legend />
                          <Bar dataKey="Delivery" fill="hsl(var(--primary))" radius={[3, 3, 0, 0]} />
                          <Bar dataKey="Content" fill="#22d3ee" radius={[3, 3, 0, 0]} />
                    ...
                    <tbody>
                      {instructors.map((inst, i) => (
                        <motion.tr
                          ...
                          className={cn("border-b border-border/20 hover:bg-white/[0.02] transition-colors", i === instructors.length - 1 && "border-0")}
                        >
                          <td className="px-6 py-4 font-semibold text-white">{inst.instructor}</td>
                          <td className="px-6 py-4 text-muted-foreground">
                            <div className="flex flex-wrap gap-1">
                              {inst.subjects.map((s) => (
                                <span key={s} className="px-2 py-0.5 bg-white/5 rounded text-xs text-white/70">{s}</span>
                              ))}
                            </div>
                          </td>
                          <td className="px-6 py-4 text-muted-foreground">{inst.total_sessions}</td>
                          <td className="px-6 py-4 text-muted-foreground">{inst.total_responses}</td>
                          <td className="px-6 py-4"><RatingBadge value={inst.avg_delivery} /></td>
                          <td className="px-6 py-4"><RatingBadge value={inst.avg_content} /></td>
                          <td className="px-6 py-4">
                            <span className="font-bold text-white">{inst.avg_rating.toFixed(2)}</span>
                          </td>
                        </motion.tr>
                      ))}
                    </tbody>
          ...
          >
            ...
            {topicsLoading ? (
              ...
              <div className="bg-card border border-border/50 rounded-2xl p-12 text-center">
                ...
                <p className="text-muted-foreground text-sm max-w-sm mx-auto">
                  Upload Format B CSV files or fill in the Topic field when uploading to track per-topic performance. You can also edit existing uploads.
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto bg-card border border-border/50 rounded-2xl shadow-xl">
                  ...
                  <tbody>
                    {topics.map((t, i) => (
                      <motion.tr
                        ...
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: i * 0.04 }}
                        className={cn("border-b border-border/20 hover:bg-white/[0.02] transition-colors", i === topics.length - 1 && "border-0")}
                      >
                        <td className="px-6 py-4 font-semibold text-white max-w-[200px]">
                          <span className="block truncate" title={t.topic}>{t.topic}</span>
                        </td>
                        <td className="px-6 py-4 text-muted-foreground">
                          <span className="px-2 py-0.5 bg-white/5 rounded text-xs text-white/70">{t.subject}</span>
                        </td>
                        <td className="px-6 py-4 text-muted-foreground">
                          {t.instructor ? (
                            <span className="text-white/80 text-sm">{t.instructor}</span>
                          ) : (
                            <span className="italic opacity-40 text-xs">Not specified</span>
                          )}
                        </td>
                        <td className="px-6 py-4 text-muted-foreground">{t.sessions}</td>
                        <td className="px-6 py-4 text-muted-foreground">{t.total_responses}</td>
                        <td className="px-6 py-4"><RatingBadge value={t.avg_rating} /></td>
                        <td className="px-6 py-4">
                          {t.feedback_count > 0 ? (
                            <span className="px-2.5 py-1 bg-primary/10 text-primary rounded-full text-xs font-medium">
                              {t.feedback_count} comments
                            </span>
                          ) : (
